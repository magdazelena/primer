...
describe("Product Series Service", () => {
  ...
  describe("createProductsFromSeries", () => {
            ...
            description: [
              {
                ...
                children: [
                  {
                    type: "text",
                    text: "Amazing wooden chair, what a great bargain",
                  },
                ],
              },
            ],
    ...
    it("should create products from series with correct data", async () => {
      const count = 3;
      const products = await service.createProductsFromSeries(
        testSeries.documentId,
        count,
      );
    ...
    it("should throw error if series not found", async () => {
      await expect(
        service.createProductsFromSeries("non-existent-id"),
  ...
  describe("updateSeriesProducts", () => {
    ...
    it("should update specified fields for all products in series", async () => {
      await strapi.db.query("api::product-series.product-series").update({
        ...
        data: {
          ...
          description: [
            {
              type: "paragraph",
              children: [{ type: "text", text: "Amazing." }],
            },
          ],
        },
      });
      ...
      // Verify updates
      const updatedProducts = await strapi.db
        .query("api::product.product")
        .findMany({
          where: {
            series: {
    ...
    it("should throw error if series not found", async () => {
      ...
      await expect(
        service.updateSeriesProducts("non-existent-id", updateData),
      ).rejects.toThrow("Series not found");
    });
    ...
    it("should handle updating all available fields", async () => {
      ...
      await expect(
        service.updateSeriesProducts(testSeries.documentId, updateData),
      ).resolves.toBe(true);
    });
  });
});
